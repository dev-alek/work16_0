...
&then
 &message необявлен источник копипрования bufSource
&endif
&if defined(bufSource) eq 0
&then
 &message необявлен Приемник копипрования bufTarget
&endif  
&if defined(tableKeyMerge) eq 0
&then
 &message необявлен не заданы поля для поиска записей tableKeyMerge
&endif  
