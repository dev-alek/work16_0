...
DO:
   DEFINE VARIABLE v-recid AS RECID NO-UNDO.
   define variable v-ok    as logical      no-undo.
   IF AVAILABLE action-role THEN DO:
      ASSIGN
   ...
   IF AVAILABLE action-role THEN DO:
      ...
      then do:
            message
               ...
               skip(1)
               skip "Ошибка изменения списка прав"
               skip return-value
...
DO:
   if available action-item then do:
      assign
...
DO:
   if (lookup  ( "b-add" , p-bttns) > 0 ) then do:
 ...
 , buf_action-role-item
   ...
   for each  buf_action-item
       ...
       find first buf_action-group no-lock
            ...
            no-error
            .
       IF NOT AVAILABLE buf_action-group THEN DO:
          next each-item_.
       END.
         ...
         then do:
               ...
               then do:
                  message
                        vss-workfile vss-revision vss-description
                     skip(1)
                     skip "Ошибка поиска прав в системе."
                     skip return-value
                     skip trim( error-status :get-message( 1 ) )
                        trim( error-status :get-message( 2 ) )
                        trim( error-status :get-message( 3 ) )
...
------------------------------------------------------------------------------*/
  ...
  ENABLE
      ...
      rs-scope  WHEN  (lookup  ( "rs-scope" , p-bttns) > 0 )
      b-toggle  WHEN  (lookup  ( "b-add" , p-bttns) > 0 )
      browse-action-role browse-action-item
   WITH FRAME Dialog-Frame.
    { gbl/chk-actg.i
      v-cntxt-db-num
