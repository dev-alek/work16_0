...
if not available buf_comp_fbr-line then do:
  message "Неправильно выбрана строка.".
  return no-apply.
end.
do
...
:
    ...
    then do:
        message
            ...
            skip "Ошибка изменения количества составного товара."
            skip return-value
    ...
    then do:
        ...
        then do:
            message
                    vss-workfile vss-revision vss-description
                skip "Ошибка расчета количества ингредиентов"
                skip "при изменении количества составного товара"
                skip return-value
                skip trim(error-status :get-message(1))
                    trim(error-status :get-message(2))
                    trim(error-status :get-message(3))
...
then do:
    ...
    :
        ...
        then do:
            message
                     vss-workfile vss-revision vss-description
                skip "Ошибка проверки соответствия"
                skip "строк рецепта документа и строк документа."
                skip(1)
...
DO:
    ...
    then do:
        message
                vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка просмотра документа производства по товарам."
            skip return-value
            skip trim( error-status :get-message( 1 ) )
                trim( error-status :get-message( 2 ) )
                trim( error-status :get-message( 3 ) )
...
then do:
    ...
    then do:
        message
                    vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка записи строки ингредиента"
            skip "документа производства"
            skip return-value
            skip trim(error-status :get-message(1))
                    trim(error-status :get-message(2))
                    trim(error-status :get-message(3))
...
DO:
    ...
    then do:
        message
            vss-workfile vss-revision vss-description
            skip "Ошибка добавления товаров по всем связанным рецептам."
            skip return-value
            skip trim( error-status :get-message(1) )
        view-as alert-box error
...
DO:
    ...
    then do:
        message
            vss-workfile vss-revision vss-description
            skip "Ошибка добавления товара без рецепта в верхний список."
            skip return-value
            skip trim( error-status :get-message(1) )
        view-as alert-box error
        title "Ошибка добавления товара".
...
DO:
    ...
    then do:
        message
                 vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка копирования накладной в документ производства."
            skip return-value
            skip trim( error-status :get-message( 1 ) )
                 trim( error-status :get-message( 2 ) )
                 trim( error-status :get-message( 3 ) )
...
DO:
    ...
    then do:
        message
          vss-workfile vss-revision vss-description
          skip "Ошибка резервирования товара по всему документу."
          skip return-value
          skip trim(error-status :get-message(1))
               trim(error-status :get-message(2))
               trim(error-status :get-message(3))
...
DO:
    ...
    then do:
        message
            vss-workfile vss-revision vss-description
            skip "Ошибка добавления товара без рецепта в нижний список."
            skip return-value
            skip trim( error-status :get-message(1) )
        view-as alert-box error
        title "Ошибка добавления товара".
...
DO:
    ...
    then do:
        message
            vss-workfile vss-revision vss-description
            skip "Ошибка добавления товара с рецептом."
            skip return-value
            skip trim( error-status :get-message(1) )
        view-as alert-box error
        title "Ошибка добавления товара".
...
DO:
    ...
    then do:
        message
                 vss-workfile vss-revision vss-description
            skip "Ошибка расчета рецепта. "
            skip "Рецепт: " buf_comp_fbr-line.recipe-code
            skip return-value
            skip trim(error-status :get-message(1))
                 trim(error-status :get-message(2))
                 trim(error-status :get-message(3))
...
DO:
    ...
    then do:
        message
                 vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка выбора оператора документа производства."
            skip return-value
            skip trim(error-status :get-message(1))
                 trim(error-status :get-message(2))
                 trim(error-status :get-message(3))
...
DO:
    ...
    then do:
        message
                 vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка выбора кода оплаты документа производства."
            skip return-value
            skip trim(error-status :get-message(1))
                 trim(error-status :get-message(2))
                 trim(error-status :get-message(3))
...
on return, mouse-select-dblclick of buf_comp_fbr-line.is-calc in browse br-comp do:
...
on return, mouse-select-dblclick of buf_ingr_fbr-line.is-calc in browse br-ingr do:
  /* инвертируем значение на экране */
...
on return, mouse-select-dblclick of buf_comp_fbr-line.fix-cost in browse br-comp do:
  /* инвертируем значение на экране */
...
on return, mouse-select-dblclick of buf_ingr_fbr-line.fix-cost in browse br-ingr do:
  /* инвертируем значение на экране */
        ...
        then do:
            ...
            then do:
                message
                        vss-workfile vss-revision vss-description
                    skip(1)
                    skip "Ошибка определения оператора производства."
                    skip(1)
                    skip "Выберите ответственного за операции производства."
                    skip return-value
                    skip trim(error-status :get-message(1))
                        trim(error-status :get-message(2))
                        trim(error-status :get-message(3))
...
define input parameter p-price-obj-code like ub.clients.obj-code       no-undo. /* код объекта для поиска прод цены */
    ...
    then do:
        ...
        message
            ...
            skip "Ингредиенты не могут совпадать с получаемыми товарами !"
            skip(1) "Товар:" buf_goods.artic buf_goods.gds-name
        view-as alert-box error
        title "Ошибка добавления товара".
        undo, return error.
            ...
            :
                ...
                then do:
                    ...
                    then do:
                        ...
                        no-error.
                        if available buf_comp_fbr-line
                        then do:    /* встаем в верхнем браусе на строку с пустым рецептом и показаваем все такие строки */
                            assign
                                v-fbr-doc-line-rec = recid (buf_comp_fbr-line)
                            .
                        end.
                        else do:    /* для включения всех строк в нижнем браусе и reposition на новую - она в v-fbr-doc-rep-rec */
                            assign
                                v-fbr-doc-line-rec = ?
                            .
                        end.
...
:
    ...
    :
        ...
        then do:
            ...
            then do:      /* изменилась цена - ставим отметку */
                assign
        ...
        then do:
            ...
            then do:
            ...
            else do:        /* редактирование учетных цен разрешено */
                ...
                then do:        /* изменилась цена или сумма НДС - ставим отметку */
                    assign
                    ...
                    then do:        /* изменилась цена - пересчитать НДС */
                        assign
                ...
                then do:
                    message
                        vss-workfile vss-revision vss-description
                        skip "Ошибка вычисления курса базовой валюты."
                        skip return-value
                        skip trim(error-status :get-message(1))
                            trim(error-status :get-message(2))
                            trim(error-status :get-message(3))
...
:
    ...
    then do:
        message
                vss-workfile vss-revision vss-description
            skip(1)
            skip "Ошибка пересчета измененной строки"
            skip "документа производства"
            skip return-value
            skip trim(error-status :get-message(1))
                trim(error-status :get-message(2))
...
define input parameter p-host-code              as integer      no-undo.
    ...
    then do:        /* для статуса Новый может измениться только цена продажи или ее фиксированность */
    ...
    then do:        /* для статуса Разрешен может измениться только учетная цена или ее фиксированность */
        ...
        */
        ...
        then do:
            ...
            then do:        /* изменилась цена - ставим отметку, что она зафиксирована */
                assign
                    buf_fbr-line.fix-cost              = yes
                .
                run cur-time in this-procedure ( output v-today
            ...
            then do:
                ...
                .
                message
                    skip "Сумма НДС учетных цен не может быть"
                    skip "больше или равна сумме учетных цен"
                    skip(1)
                    skip "Введите верные данные."
                view-as alert-box error.
                undo, return error .
            end.
        end.        /* редактирование учетных цен разрешено */
...
:
    ...
    then do:
        ...
        message
            ...
            skip(1)
            skip "Вы можете:"
            skip "    Изменить рецепт документа в соответствии"
            skip "        с введенными значениями количеств ингредиентов."
            skip "    или вернуться к редактированию документа,"
            skip "        чтобы привести строки в соответствие рецепту"
            skip "        кнопками Составной и Ингредиенты"
            skip(1)
            skip "Изменить рецепт документа?"
        view-as alert-box question
        buttons ok-cancel
...
case rs-one-all :
    ...
    then do:
        ...
        then do:    /* верхний браус не пуст - нижний по рецепту будет работать правильно */
            if available buf_comp_fbr-line
            then do:
                { gbl/fltopend.i
                    &where-cond = "buf_ingr_fbr-line.doc-code = f-doc.doc-code and ~
    ...
    :
        ...
        then do:
            message
                ...
                skip "не была создана соответствующая строка в складском документе списания."
                skip(1)
                skip "Просмотр партий невозможен."
            view-as alert-box warning.
            undo, return error.
        end.
        assign
        ...
        then do:
            ...
            then do:
                message
                        vss-workfile vss-revision vss-description
                    skip "Ошибка при вызове интерфейса работы с партиями товара."
                    skip return-value
                    skip trim(error-status :get-message(1))
                        trim(error-status :get-message(2))
                        trim(error-status :get-message(3))
        ...
        then do:
            message
                ...
                skip "было изменено общее количество товара в документе."
                skip(1)
                skip "Произвести такое изменение количеств по партиям невозможно."
                skip "Операция будет отменена."
            view-as alert-box error.
            undo, return error.
        end.
        assign
...
:
    ...
    then do:
        message
                 vss-workfile vss-revision vss-description
            skip(1)
            skip "Невозможно отобразить имя оператора производства."
            skip return-value
            skip trim(error-status :get-message(1))
                 trim(error-status :get-message(2))
                 trim(error-status :get-message(3))
    ...
    then do:
        ...
        case rs-one-all :
            when "goods"
            then do:        /* проверяем доступность товара для фильтрации */
    ...
    /*            buf_ingr_fbr-line.price-sum-rubl:read-only in browse br-ingr = yes*/
        ...
        if rs-one-all = "goods"
        then do:        /* не знаю, какие запрещать, закрою на всякий случай эти */
            disable
                b-add
                b-del
            with frame {&frame-name}.
    ...
    then do:
        ...
        and rs-one-all <> "all"
        then do:      /* требуется встать на строчку в нижнем browse для пустого рецепта - в верхнем может */
    ...
    if available buf_comp_fbr-line then do:
      ...
      if available buf_ingr_fbr-line then do:
        current-browse = br-ingr :handle.
      end.
    end.
    if v-fbr-doc-rep-rec <> ?
