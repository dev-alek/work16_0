...
define temp-table tt-chk no-undo
    ...
    field sum-gds-ban-bonus_ as decimal                 /* Сумма по товарам с глобальным атрибутом "Запрет на участие в бонусных программах»". */
...
/*/* Служебный */ my-table.obj-code = string(ub.chk-doc.obj-code).                          */
    ...
    do:  /* Вычисления и запись данных в tt_chk по "Уровню Минимального Ункального Ключа Отчёта(УМУКО)". */
        ...
        /*A**************************************/
        do:  /* Пробежим по иниям оплаты товаров в Чеке (в ub.chk-gds-pay) и: 1) найдём товар с глобальным атрибутом «Запрет на участие в бонусных программах», сохраним его в v-ban-bonus; 2) найдём тип платежа: "Оплата баллами Малина", сохраним его в v-bonus-malina; чтобы по ТЗ исключить их ниже из суммы чека в этом отчёте */
...
/*                                /* Служебный */ view-as alert-box.*/
                    ...
                    /* Нач. Найдём товар с глобальным атрибутом «Запрет на участие в бонусных программах». */
                    ...
                    :
                        ...
                        do:
                            if v-calc-ban-bonus = no then /* учтём товары с атрибутом «Запрет на участие в бонусных программах», только если они не учтены в проверке выше (см "Оплата баллами Малина") */
                            do:
...
/*/* Служебный */ my-table.list-bonus-malina = my-table.list-bonus-malina + "; " + string(v-bonus-malina).                               */
                        /* КНЦ. Найдём товар с глобальным атрибутом «Запрет на участие в бонусных программах». */
