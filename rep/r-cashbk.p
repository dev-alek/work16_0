...
run report-exec in this-procedure (input v-date-start ).
...
do ii = 1 to num-entries (p-cashbook,{&delim-cmd}):
  ...
  put stream OutStr-html unformatted
    '<TABLE fit_to_page="true" orientation="portrait" CELLSPACING="0" BORDER="0" name="Кассовая книга ' + if available (temp-fin-doc) then string(temp-fin-doc.cashbook_id) + '">' else string(buf_cashbook.id) + '">'skip
    .
  ...
  put stream OutStr-html unformatted
    ...
    '</tr>' skip
    '<tr>' skip
    '<td colspan="5">' + v-obj-name + '</td>' skip
    '</tr>' skip
    '<tr>' skip
    '<td colspan="5">' + "Кассовая книга " + if available (temp-fin-doc) then temp-fin-doc.cashbook  + " Касса за " + v-date-name + v-tab110 + "Лист____" + '</td>' else buf_cashbook.CashBookName + " Касса за " + v-date-name + v-tab110 + "Лист____" + '</td>' skip
    '</tr>' skip
    '</thead>' skip
    '<tbody>' skip
    .
  ...
  put stream OutStr-html unformatted
    '<tr>' skip
    '<th style="width: 120px; align: center;">Номер документа</th>' skip
  ...
  put stream OutStr-html unformatted
    '<tr>' skip
    '<td style="align: center;">1</td>' skip
    '<td style="align: center;">2</td>' skip
    '<td style="align: center;">3</td>' skip
    '<td style="align: center;">4</td>' skip
    '<td style="align: center;">5</td>' skip
    '</tr>' skip
    '<tr>' skip
    '<td></td>' skip
    '<td colspan="2" style="align: right;">Остаток на начало дня</td>' skip
    '<td style="align: center;">' + if available (temp-fin-doc) then string(temp-fin-doc.ostatok-begin, "->>>>>>>>9.99") + '</td>' else "" + '</td>' skip
    '<td style="align: center;">Х</td>' skip
    '</tr>' skip
    .
  ...
  do:
    for each buf_temp-fin-doc where buf_temp-fin-doc.cashbook_id = temp-fin-doc.cashbook_id:
      put stream OutStr-html unformatted
        '<tr>' skip
        '<td style="align: center;">' + buf_temp-fin-doc.prn-doc-code + '</td>' skip
        '<td text_wrap="true" style="align: center;">' + if buf_temp-fin-doc.fin-doc-type = {&income-cash} then buf_temp-fin-doc.payer  + '</td>' else buf_temp-fin-doc.receiver + '</td>' skip
        '<td text_wrap="true" style="align: center;">' + buf_temp-fin-doc.cor-acc + '</td>' skip
        '<td text_wrap="true" style="align: center;">' + if buf_temp-fin-doc.fin-doc-type = {&income-cash} then string(buf_temp-fin-doc.sum-rubl, "->>>>>>>>9.99")  + '</td>' else "         -" + '</td>' skip
        '<td text_wrap="true" style="align: center;">' + if buf_temp-fin-doc.fin-doc-type = {&expense-cash} then string(buf_temp-fin-doc.sum-rubl, "->>>>>>>>9.99")  + '</td>' else "         -" + '</td>' skip
        '</tr>' skip
        '<tr>' skip
        .
    end.
  ...
  do:
    ...
    put stream OutStr-html unformatted   
      '<tr>' skip         
      '<td colspan="3" style="align: right;">Итого за день</td>' skip
      '<td>' + if available (temp-fin-sum) then string(temp-fin-sum.sum-income, "->>>>>>>>9.99") + '</td>' else "" + '</td>' skip
      '<td>' + if available (temp-fin-sum) then string(temp-fin-sum.sum-expense, "->>>>>>>>9.99") + '</td>' else "" + '</td>' skip
      '</tr>' skip 
      '<tr>' skip
      '<td text_wrap="true" colspan="3" style="align: right;">Остаток на конец дня, в том числе на заработную плату, выплаты социального характера и стипендии</td>' skip
      '<td>' + if available (temp-fin-sum) then string((temp-fin-sum.ostatok), "->>>>>>>9.99") + '</td>' else "" + '</td>' skip
      '<td>Х</td>' skip
      '</tr>' skip
  ...
  put stream OutStr-html unformatted
    ...
    '<tr>' skip
    '<td>Кассир</td>' skip
    '<td style="align: center;">_________________</td>' skip
    '<td></td>' skip
    '<td colspan="2" style="align: center;">_______________________</td>' skip
    '</tr>' skip
    '<tr>' skip
    '<td></td>' skip
    '<td style="align: center;">подпись</td>' skip
    '<td></td>' skip
    '<td colspan="2" style="align: center;">подпись</td>' skip
    '</tr>' skip
    '<tr>' skip
    '<td colspan="3">Записи в кассовой книге проверил и документы в количестве</td>' skip
    '<td></td>' skip
    '<td></td>' skip
    '</tr>' skip
    '<tr>' skip
    '<td colspan="3">' + v-pko-propis + " приходных и " + v-rko-propis + " расходных получил." + '</td>' skip
    '<td></td>' skip
    '<td></td>' skip
    '</tr>' skip 
    '<tr>' skip
    '<td>Бухгалтер</td>' skip
    '<td style="align: center;">_________________</td>' skip
    '<td></td>' skip
    '<td colspan="2" style="align: center;">_______________________</td>' skip
    '</tr>' skip
  ...
  for each buf_obj-list no-lock :
    ...
    do ii = 1 to num-entries (p-cashbook,{&delim-cmd}):
      ...
      run fostatok in this-procedure (
        ...
        ,input   yes /*xTog-obj*/
        ,input   0 /*p-curr-code*/
        ,input   integer(entry(ii,p-cashbook,{&delim-cmd}))
        ,output  v-sum-begin
        ,output  Fact-order-1)
        no-error .
      run fostatok in this-procedure (
