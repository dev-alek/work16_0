...
IF NOT AVAILABLE ub.sysconf THEN DO:
  return error substitute("&1 &2 &3&4Не найдена фирма типа &5 СМН объект &6&7 смена &8 порядок &9"
                          ...
                          ,pshift-num).
END.
ASSIGN p-base-code = ub.sysconf.base-code.
...
/*             previous-rvs-line.pl-code  = ub.rvs-line.pl-code       AND*/
      ...
      IF found-in-previous = NO THEN DO:
      ...
      if available temp-rvs then do:
        if temp-rvs.shift-date < ub.rvs-doc.shift-date or temp-rvs.shift-date = ub.rvs-doc.shift-date and temp-rvs.shift-num  <= ub.rvs-doc.shift-num then do:
          ASSIGN
  ...
  IF moving <> YES THEN DO: RETURN. END.
  ...
  , FIRST t-2                 WHERE
    ...
    ASSIGN for-supp-name = ( IF AVAILABLE ub.clients THEN ub.clients.obj-name ELSE "":U ) .
      ...
      IF AVAILABLE ub.clients THEN DO:
        ...
        IF NOT ERROR-STATUS :ERROR THEN DO:
          ...
          FOR EACH tt-ot-line NO-LOCK WHERE
            ...
            IF tt-ot-line.ext-doc-type <> {&TDEDT_Pri_Vnesh} THEN DO:
              ASSIGN
                acc-cli-found = YES
              ...
              :
                IF tt-ot-line.ext-doc-type = {&TDEDT_Inv} or tt-ot-line.ext-doc-type = {&TDEDT_Peresort} THEN DO:
                  ASSIGN acc-cli-qnty2 = acc-cli-qnty2 + ( IF buf_doc-line.cli-qnty = ? THEN 0 ELSE buf_doc-line.cli-qnty ).
                END.
                ELSE DO:
                  ...
                  ASSIGN acc-cli-qnty2 = acc-cli-qnty2 + ( IF AVAILABLE buf_inv-line THEN ( IF buf_inv-line.wast-cli-qnty = ? THEN 0 ELSE ( - buf_inv-line.wast-cli-qnty ) ) ELSE 0 ).
                END.
              END. /* FOR EACH buf_doc-line */
            END. /* IF tt-ot-line.ext-doc-type <> {&TDEDT_Pri_Vnesh} */
          END. /* FOR EACH tt-ot-line */
          IF acc-cli-found = YES THEN DO:
...
_trn-doc:
  FOR EACH  ub.trn-doc NO-LOCK WHERE
    ...
    , FIRST ub.doc-line NO-LOCK WHERE
            ...
            ub.doc-line.prod-code = t-2.prod-code
  :
    if ub.trn-doc.ext-doc-type = {&TDEDT_Spi_Vnesh} then do:
      RUN clntattr-value IN THIS-PROCEDURE (  INPUT ub.trn-doc.cli-type, INPUT ub.trn-doc.cli-code, INPUT {&attr-shftrep2}, OUTPUT v-attr-value, OUTPUT v-attr-type ).
    ...
    IF AVAILABLE ub.inv-line THEN DO:
      ASSIGN acc-other-qnty2 = acc-other-qnty2 + ( IF ub.trn-doc.ext-doc-type = {&TDEDT_Vozvrat_Vnesh} THEN ( - ub.inv-line.wast-cli-qnty ) ELSE ub.inv-line.wast-cli-qnty ).
    END.
  END. /* FOR EACH ub.trn-doc */
  IF acc-other-found = YES THEN DO:
    RUN create-treal-2 IN THIS-PROCEDURE ( INPUT t-2.gds-code
                                         , INPUT -1
                                         , INPUT 0
