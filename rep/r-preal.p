...
put stream OutStr-html unformatted
   '<body>' skip
   '<table orientation="landscape" name = "Отчет по пересменкам" fit_to_page="true">' skip  /* таблица, в которой содержится весь отчет */
   '<thead>' skip
...
PROCEDURE itog_azs:
   ...
   if time-prev-azs > 0 then time-pr-sv = time-prev-azs / kol-per-azs.  /* средняя длительность простоя с превышением  */
...
PROCEDURE itog_azs-all:
   ...
   if time-prev-azs-all > 0 then time-pr-sv-all = time-prev-azs-all / kol-per-azs-all.  /* средняя длительность простоя с превышением  */
