  ...
  IF CAN-FIND( FIRST treal-2 WHERE
    ...
    do v-step = 1 to 2:
      ...
      FOR EACH treal-2 WHERE treal-2.is-pay = v-is-pay AND treal-2.gds-code = t-2.gds-code and treal-2.curr-code >= 0 USE-INDEX pi :
        if treal-2.discnt-type = -99 or treal-2.discnt-type = -98 or (treal-2.cpay-code < 0) then do: 
        assign
        ...
        if treal-2.is-pay = yes then do:
          ...
          curr-real-ii         = curr-real-ii + 1.
          if treal-2.cpay-code <> -4 and treal-2.discnt-type <> -98 then do: /* инвентаризацию и техпролив с сервисным платежем не включаем */
  ...
  DO jj = 1 TO t-2.lines :
    ...
    IF jj = 1 then do:
      assign pol1      = t-2.gds-name
             pol2      = t-2.last-price
             pol3      = t-2.qnty1-before
             pol4      = t-2.qnty2-before
             pol18     = t-2.qnty1-after
             pol19     = t-2.qnty2-after
             main-line = yes.
      if p-batch > 0
      then do:
    ...
    IF AVAIlABLE tincome-2 THEN DO:
      ...
      ASSIGN pol5      = tincome-2.supp-name
...
'<td text_wrap="true" num="0.00" val="' + fnc-convert-dot-to-colon(pol19,"->>>>>>>>>>>>>9.99",2) + '" style="text-align: right;">' + if main-line = yes then fnc-convert-dot-to-colon(pol19,"->>>>>>>>>>>9.99",2) + '</td>' else "" + '</td>' skip
'</tr>' skip
.
...
/*              release buf_shift-pgds-out.*/
        ...
        put stream OutStr-html unformatted
         ...
         put stream OutStr-html unformatted
            ...
            if pay-line = no or pol15 = 0 then "" else string(pol15,"->>>>>>>>>>>9.99"),
            if pay-line = no or pol16 = 0 then "" else string(pol16,"->>>>>>>>>>>9.99"),
            if pay-line = no or pol17 = 0 then "" else string(pol17,"->>>>>>>>>>>9.99"),
			if main-line = no then "" else string(pol18,"->>>>>>>>>>>9.99")
            ).
put stream OutStr-html unformatted
  ...
  IF LAST( t-2.main-code ) THEN DO:
    ...
    assign pol1  = "ИТОГО"
        ...
        put stream OutStr-html unformatted
            ...
            '  <tr>
                    ...
                    <th style="text-align: left; vertical-align: middle;">&8</th>
                    <th style="text-align: left; vertical-align: middle;">&9</th>
                    '
            ,
            pol1,
            string(pol3,"->>>>>>>>>>>9.99"),
        ...
        put stream OutStr-html unformatted
            substitute (
            '
            <th style="text-align: right; vertical-align: middle;">&1</th>
    ...
    if can-find( first actreal-2 no-lock ) then do:
      ...
      FOR EACH actreal-2 NO-LOCK
        ...
        assign pol11 = actreal-2.out-name
        ...
        if last-of( actreal-2.is-pay ) /* and actreal-2.is-pay <> ? */ then do:
          if actreal-2.discnt-type = -99 or actreal-2.cpay-code < 0 then do:
          if actreal-2.is-pay = yes then do:
            assign pol11 = "ИТОГО ОПЛАЧ.РАСХОД"
                   pol12 = areal-is-pay-qnty1
                   pol13 = areal-is-pay-qnty2
                   pol14 = areal-is-pay-netto.
          end.
          else do:
            assign pol11 = "ИТОГО ПРОЧ.РАСХОД"
