...
define frame f-doc
        ...
        sym3 column-label ":"  format "X(1)" space(0)   v-goods-name     COLUMN-LABEL "Наименование продуции"          format "X(38)" space(0)
        sym4 column-label ":"  format "X(1)" space(0)   v-blank-num      COLUMN-LABEL "№ Бланка"                       format "X(14)" space(0)
        sym5 column-label ":"  format "X(1)" space(0)   v-sert-num       COLUMN-LABEL "Регистрационный номер"          format "X(35)" space(0)
        ...
        sym8 column-label ":"  format "X(1)" space(0)   v-sert-org       COLUMN-LABEL "Орган выдавший сертификат"      format "X(39)" space(0)
    ...
    HEADER
        ...
        v-single-line format "X(182)" AT 1
...
form header
    v-single-line format "X(182)" skip "Продолжение - на следующей странице" at 30 skip
...
put stream Out-Stream
    skip    v-single-line                   format "X(182)"
    skip(2) space(5)  "Оригиналы сертификатов или их заверенная копия находятся у: " + v-org-name  format "X(150)"
    skip(2) space(5)  "Уполномоченный представитель " + buf_clients.obj-name + v-contact-name + " " + string(v-underline, "X(20)") + " М.П."  format "X(150)"
.
...
run r-sertxl-write-cell-data in this-procedure ( input {&r-sertxl-f_contact}, input string("Уполномоченный представитель " + buf_clients.obj-name + v-contact-name + "                                           М.П.")).
