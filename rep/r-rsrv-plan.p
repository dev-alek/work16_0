...
for each gds-list:
  ...
  do:
    if pDelDayGoods then tt-zakaz.tempSale = round-maxDec(tt-zakaz.volSale / tt-zakaz.qntyDayGoods) . /* Тпр */
...
for each tt-zakaz no-lock break by tt-zakaz.contract by tt-zakaz.gds-code:
  ...
  if tt-zakaz.qntyDayGoods = 0 or tt-zakaz.qntyDaySale = 0 then do:
  put stream OutStr-html unformatted
    '       <tr>' skip
...
procedure ost-gds-day :
  do
    ...
    assign
      temp-gds-qnty.day      = today
      temp-gds-qnty.ost      = p-ost-today
