          ...
          and not can-find (first buf_person where buf_person.inn = src_firm.inn) then do:
            ...
            then do:
              if available src_temp-sysconf then do:
                &scop my-message substitute("Импорт &1&2: в СИСТЕМЕ &5 это СВОЯ ФИРМА , а в v15.1  (&3&4) - НЕТ" ~
                ...
                &scop my-message substitute("Импорт &1&2: в v15.1 это СВОЯ ФИРМА (&3&4), а в &5  - НЕТ" ~
                                          , src_clients.obj-type ~
                                          , src_clients.obj-code ~
                                          , ~{&cmp~} ~
                                          , buf_firm.firm-code ~
