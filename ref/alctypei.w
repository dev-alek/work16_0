...
DO:
   ...
   if p-def = {&add-def} then do:
      ...
      THEN DO:
         MESSAGE "Введенный код типа алкоголя уже существует" buf_alc-type.alc-type-name " ! " VIEW-AS  ALERT-BOX  ERROR.
   ...
   if p-def = {&add-def} OR p-def = {&update}  then DO:
      if v-name = "" then do:
         message "Введите название типа алкоголя! "
      ...
      if v-code = "" then do:
         message "Введите код типа алкоголя! "
         view-as  alert-box  error.
         apply "entry"  to v-code .
         return no-apply.
      end.
...
assign frame {&frame-name}:title = p-def + " типа алкоголя  " .
