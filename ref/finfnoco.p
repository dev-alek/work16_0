...
:
  ...
  then do:
    ...
    if not avail ttc-fin-doc then do:
  ...
  CASE p-mode:
    ...
    otherwise do:
      ...
      p-mode = {&update}
      .
      if tt-fin-doc.status_ = {&fin-permitted}  then do:
        assign
        p-limit-access = 1
        .
      end.
      if tt-fin-doc.status_ = {&fin-bank}  then do:
        assign
        p-limit-access = 2
        .
      end.
...
/*    end.                                                                                                      */
    ...
    and v-obj-db-num = v-cntxt-db-num then do:
  ...
  or (p-mode <> {&add-def}
  and tt-fin-doc.contract-code <> 0) then do:
  ...
  then do:
    FIND FIRST buf_payer NO-LOCK WHERE
            ...
            AND buf_payer.obj-code = (if p-mode = {&add-def}
                                      ...
                                      NO-ERROR .
    if not avail buf_payer then do:
    ...
    if buf_payer.obj-type = {&cmp} then do:
  ...
  then do:
    ...
    if buf_receiver.obj-type = {&cmp} then do:
  ...
  or p-fin-doc-type = {&expense-cashless} then do:
  ...
  if tt-fin-doc.status_ <> {&fin-fact} then do:
    if tt-fin-doc.obj-type <> ''then do:
/*      run adm/shattri.p (                 */
/*          input "get":U                   */
/*          ,input  tt-fin-doc.obj-type     */
/*          ,input  tt-fin-doc.obj-code     */
...
/*        end case.                                       */
    ...
    CASE p-fin-ext-doc-type:
      ...
      when {&FDEDT_Income_Payoff} then do:
        assign
        tt-fin-doc.receiver-type = {&cmp}
        tt-fin-doc.receiver-code = p-host-code
        tt-fin-doc.receiver-okpo = buf_firm.okpo
  ...
  if tt-fin-doc.contract-code <> 0 then do:
    ...
    .
      ...
      or  (p-fin-doc-type = {&expense-cashless}
        ...
        and buf_payer-fin-schet.status_ <> {&current-status} then do:
        ...
        and buf_payer-fin-bank.status_ <> {&current-status} then do:
      ...
      then do:
        ...
        and buf_receiver-fin-schet.status_ <> {&current-status} then do:
        ...
        and buf_receiver-fin-bank.status_ <> {&current-status} then do:
          undo, return error substitute("Статус банка с кодом &1 &2"
  ...
  if tt-fin-doc.status_ <> {&fin-fact} then do:
    ...
    and  tt-fin-doc.fin-ext-doc-type <> {&FDEDT_income_cashless} then do:
      run tempchgs-create-lable-record in this-procedure (
                                                          input {&table_fin-doc}
 ...
 or (p-mode = {&add-def}
  ...
  if p-mode = {&add-def} then do:
    for each buf_tt-fin-doc no-lock where
      ...
      if not available tt0-fin-doc then do:
        create tt0-fin-doc.
      end.
  ...
  if p-mode = {&update} then do:
    for each bufc_ttc-fin-doc no-lock where
      ...
      if not available tt0c-fin-doc then do:
        create tt0c-fin-doc.
      end.
...
define variable v-cel-nazn-value like ub.fin-doc.cel-nazn-value no-undo .
  ...
  or tt-fin-doc.fin-ext-doc-type = {&expense-payoff} then do:
    if p-cor-acc1 <> 0 then do:
      ...
      and buf_fin-code-cor-acc1.status_ = integer({&current-status-int}) then do:
        assign
...
define variable v-refill-receiver as logical no-undo .
  ...
  :
    ...
    or (available buf_payer-fin-schet and p-payer-code-schet <> buf_payer-fin-schet.code-schet) then do:
      if p-payer-code-schet = 0
      or p-payer-code-schet = ? then  do:
        ...
        if available buf_payer-fin-schet then do:
          ...
          or buf_payer-fin-bank.code-bank <> buf_payer-fin-schet.code-bank then do:
            find first buf_payer-fin-bank no-lock where
                      buf_payer-fin-bank.host-code = p-host-code
                AND  buf_payer-fin-bank.code-bank = buf_payer-fin-schet.code-bank no-error .
          end.
    ...
    or (available buf_receiver-fin-schet and p-receiver-code-schet <> buf_receiver-fin-schet.code-schet) then do:
      if p-receiver-code-schet = 0
      or p-receiver-code-schet = ? then  do:
        ...
        if available buf_receiver-fin-schet then do:
          ...
          or buf_receiver-fin-bank.code-bank <> buf_receiver-fin-schet.code-bank then do:
            find first buf_receiver-fin-bank no-lock where
    ...
    if p-refill-if-nees then do:
      if v-refill-payer then do:
        assign
