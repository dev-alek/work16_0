...
_ii:
  ...
  if kk < num-entries(loc-rid-list) then do:
    message
...
DO:
  ...
  IF  X_pl-gds.tolerance :read-only in browse {&BROWSE-NAME} = FALSE THEN do:
...
b-add when lookup("b-add", bttns) > 0 and p-mode = {&place}
b-del when lookup("b-add", bttns) > 0 and p-mode = {&place}
b-chg when lookup("b-add", bttns) > 0 and p-mode = {&place}
B-sch
B-Help
b-hist
BR-pl-gds
...
CASE p-mode:
    ...
    when {&goods} or when {&petrolium} then do:
