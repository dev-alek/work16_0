...
if input frame {&frame-name} tt-fin-doc.shift-name <> tt-fin-doc.shift-name then do:
  if input frame {&frame-name} tt-fin-doc.shift-date <> ? then do:
...
define buffer bf_shift-obj   for ub.shift-obj.
if input frame {&frame-name} tt-fin-doc.shift-num <> tt-fin-doc.shift-num then do:
  if input frame {&frame-name} tt-fin-doc.shift-date <> ? then do:
  ...
  :
    for each buf_fin-schet no-lock where
      ...
      if buf_fin-schet.status_ = {&current-status} then do:
        ...
        if avail buf_fin-bank and buf_fin-bank.status_ = {&current-status} then do:
          assign
...
DO:
 { gbl/stdbtn.i }
  if   input frame {&frame-name} tt-fin-doc.obj-code <> 0 then do:
...
if tt-fin-doc.shift-flag = integer({&fin-flag-shift}) then do:
  ...
  { gbl/curshift.i
    ...
    if error-status :error then do:
      if not v-can-back-shift then do:
...
DO:
  ...
  .
  display
  tt-fin-doc.sum-rubl          when v-rubf
  tt-fin-doc.sum-doc
  tt-fin-doc.exch-rate         when v-exchf
  tt-fin-doc.exch-scale        when v-exchf
  tt-fin-doc.sum-base          when v-basef
  tt-fin-doc.base-rate         when v-baseratef
  tt-fin-doc.base-scale        when v-baseratef
  tt-fin-doc.sum-contr         when v-contractf
  tt-fin-doc.contract-rate     when v-contractratef
  tt-fin-doc.contract-scale    when v-contractratef
  f-rest-con-sum
  with frame {&frame-name} .
END.
...
if buf_fin-schet.curr-code <> tt-fin-doc.curr-code then do:
  message
...
define variable v-status_ like ub.fin-schet.status_ no-undo init {&current-status}.
define buffer buf_fin-schet for ub.fin-schet.
if not available X_{&cli-side} then do:
  message
...
FIND FIRST buf_fin-schet WHERE
      recid (buf_fin-schet) = ref-rec NO-LOCK  .
if tt-fin-doc.curr-code <> buf_fin-schet.curr-code then do:
  message
...
DO:
{ gbl/stdbtn.i }
  if   input frame {&frame-name} tt-fin-doc.{&cli-side}-code <> 0 then do:
...
if rid-list <> "":U then do:
    ...
    if X_fin-code-an-uchet.status_ <> integer({&current-status-int}) then do:
      message
...
if rid-list <> "":U then do:
    ...
    if X_fin-code-cel-nazn.status_ <> integer({&current-status-int}) then do:
      message
...
if rid-list <> "":U then do:
    ...
    if X_fin-code-cor-acc.status_ <> integer({&current-status-int}) then do:
      message
...
on leave of tt-fin-doc.shift-date in frame {&frame-name} do:
  if input frame {&frame-name} tt-fin-doc.shift-date <> tt-fin-doc.shift-date then do:
    assign
    tt-fin-doc.shift-name = ""
...
on return of tt-fin-doc.shift-date in frame {&frame-name} do:
  apply "entry" to tt-fin-doc.shift-name in frame {&frame-name}.
  return no-apply.
end.


on leave of tt-fin-doc.shift-num  in frame {&frame-name} do:
...
on leave of tt-fin-doc.shift-name in frame {&frame-name} do:
...
if rid-list <> "":U then do:
    ...
    if X_fin-code-cor-acc1.status_ <> integer({&current-status-int}) then do:
      message
      "Нельзя выбрать удаленный корр счет"
      view-as alert-box error .
      return no-apply.
...
tt-fin-doc.curr-code.
    ...
    if tt-fin-doc.curr-code <> v-curr-code then do:
...
{ gbl/stdbtn.i }
  ...
  &if "{&doc-type}" = "income-cash" or "{&doc-type}" = "expense-cash"  &then
  if tt-fin-doc.shift-flag = integer({&fin-flag-shift}) then do:
    if tt-fin-doc.shift-date >  input frame {&frame-name}  tt-fin-doc.doc-date  then do:
      message
...
if v-limit-access > 0 then do:
  ...
  tt-fin-doc.exch-rate           when v-exchf
  tt-fin-doc.exch-scale          when v-exchf
  tt-fin-doc.base-rate           when v-basef
  tt-fin-doc.base-scale          when v-basef
  tt-fin-doc.sum-contr           when v-contractf
  tt-fin-doc.contract-rate       when v-contractratef
  tt-fin-doc.contract-scale      when v-contractratef
  with frame {&frame-name}.
...
if p-main-widget = "curr-code":u then do:
  assign
  frame {&frame-name}
  tt-fin-doc.sum-doc
  .
...
if p-main-widget  = "doc-date":U then do:
  assign
  frame {&frame-name}
  tt-fin-doc.sum-doc
  .
  ...
  { gbl/exchrate.i  tt-fin-doc.curr-code tt-fin-doc.doc-date tt-fin-doc.exch-rate tt-fin-doc.exch-scale v-curr-abbr }
  { gbl/exchrate.i  tt-fin-doc.contract-curr tt-fin-doc.doc-date tt-fin-doc.contract-rate tt-fin-doc.contract-scale v-curr-abbr }
  if tt-fin-doc.base-rate:visible in frame {&frame-name} then do:
    display
    tt-fin-doc.base-rate
    tt-fin-doc.base-scale
    with frame {&frame-name}.
  ...
  if tt-fin-doc.exch-rate:visible in frame {&frame-name} then do:
    display
    tt-fin-doc.exch-rate
    tt-fin-doc.exch-scale
    with frame {&frame-name}.
  ...
  if tt-fin-doc.contract-rate:visible in frame {&frame-name} then do:
    display
    tt-fin-doc.contract-rate
    tt-fin-doc.contract-scale
    with frame {&frame-name}.
...
/*      .*/
    ...
    &if "{&doc-type}" = "income-cash" &then
      ...
      if num-entries(tt-fin-doc.including, "@":U) < 2 and p-start = 2 then do:
        assign
    ...
    &if "{&doc-type}" = "expense-cashless" &then
      ...
      .
      if p-start = 1 then p-start = 0.
      if num-entries(tt-fin-doc.naznach-plat, "@":U) < 2 and p-start = 2 then do:
        assign
