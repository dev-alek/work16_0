...
if not can-find(first temp-attr) then do:
...
CASE par-subject:
  ...
  when {&table_clients-attr} then do:
    if not can-find(first cli-list) then do:
  ...
  END.
END CASE.
if v-not-all-ok and v-delete-ok then do:
  assign
  b-list:width = 30
