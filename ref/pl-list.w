...
if available buf_place and input frame {&frame-name} X_place.PS <> buf_place.PS then do:
  assign X_place.PS.
end.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  { gbl/conf-rd.i "'is-ptrl':U"
              ...
              v_is-ptrl
              par-type      no-error }
  if error-status :error or par-type  <> {&type-log} or v_is-ptrl <> 'yes':U then do: assign v_is-ptrl = 'no':U. end.
 p-rid-list = v-rid-list.
...
ENABLE
b-quit
b-rest when can-do ("b-add", bttns)
b-print
b-help
br-pl
X_place.PS
...
b-level when can-do ("b-add", bttns)
b-sch
b-hist
