...
DEFINE VARIABLE mZNACH AS INTEGER FORMAT ">>>>>>9":U INITIAL 0 
     LABEL "Значение" 
     VIEW-AS FILL-IN 
     SIZE 10 BY 1 NO-UNDO.
...
DEFINE FRAME Dialog-Frame
     ...
     mDATA AT ROW 3 COL 18.13 COLON-ALIGNED WIDGET-ID 14
     mZNACH AT ROW 4.5 COL 18 COLON-ALIGNED WIDGET-ID 12
     SPACE(22.24) SKIP(0.99)
...
DO:
  ...
  mZNACH    .
    ...
    if p-mode = {&add-def} OR p-mode = {&update}  then
       assign
          d-code.codevalue = string(mZNACH)
          d-code.code = string(mDATA)
          p-rid = recid(d-code)
          .
     else p-rid = ? .
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   if p-mode = {&update} then do:
      find first d-code where
                 recid(d-code) = p-rid exclusive-lock no-wait no-error.
      if locked d-code then do:
         message
            vss-workfile vss-revision vss-description skip
      ...
      if not available d-code then do:
         message
            vss-workfile vss-revision vss-description skip
...
------------------------------------------------------------------------------*/
  DISPLAY mDATA mZNACH 
      WITH FRAME Dialog-Frame.
  ENABLE B-save b-quit B-Help mDATA mZNACH 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
   ...
   on stop undo, return error:
      if p-mode = {&add-def} then do:
         find first d-code where
                    d-code.parent = p-parent
                and d-code.codevalue   = string(mZNACH)
                and d-code.code   = string(mDATA)
                and recid(d-code) ne p-rid
