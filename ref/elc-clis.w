...
Коды клиентов в системе ЭЛКОС-ТАЛОН
...
define variable vss-description as character no-undo init "Коды клиентов в системе ЭЛКОС-ТАЛОН".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
define variable filter-label     as character NO-UNDO INIT "Коды клиентов в системе ЭЛКОС-ТАЛОН".
define variable filter-point0     as character NO-UNDO INIT "elc-clis".
define variable filter-label0     as character NO-UNDO INIT "Коды клиентов в системе ЭЛКОС-ТАЛОН".
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME br-elcos-cli

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES X_ext-classif X_clients

/* Definitions for BROWSE br-elcos-cli                                  */
&Scoped-define FIELDS-IN-QUERY-br-elcos-cli mark-string(recid(X_ext-classif), v-rid-list) X_ext-classif.KEY#_one X_clients.obj-type X_clients.obj-code X_clients.obj-name
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-elcos-cli
&Scoped-define SELF-NAME br-elcos-cli
&Scoped-define QUERY-STRING-br-elcos-cli FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-elcos-cli OPEN QUERY {&SELF-NAME} FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-elcos-cli X_ext-classif X_clients
&Scoped-define FIRST-TABLE-IN-QUERY-br-elcos-cli X_ext-classif
&Scoped-define SECOND-TABLE-IN-QUERY-br-elcos-cli X_clients


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-br-elcos-cli}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS b-quit B-mark B-sel b-add b-del b-cli b-sch ~
b-print B-Help br-elcos-cli mark-num
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-elcos-cli FOR X_ext-classif, X_clients SCROLLING.

&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE br-elcos-cli
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-elcos-cli Dialog-Frame _FREEFORM
  QUERY br-elcos-cli NO-LOCK DISPLAY
      mark-string(recid(X_ext-classif), v-rid-list) COLUMN-LABEL "" FORMAT "X(1)"
X_ext-classif.KEY#_one  COLUMN-LABEL "Код в!ЭЛКОС-ТАЛОН" FORMAT ">>>>>>>>9"
...
DEFINE FRAME Dialog-Frame
     ...
     br-elcos-cli AT ROW 2.88 COL 1.5 WIDGET-ID 100
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-elcos-cli B-Help Dialog-Frame */
ASSIGN
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-elcos-cli
/* Query rebuild information for BROWSE br-elcos-cli
     _START_FREEFORM
...
DO:
  ...
  if available X_ext-classif then do:
    ...
    loc#log = br-elcos-cli:refresh() .

    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-elcos-cli:select-next-row ().
        apply "VALUE-CHANGED" to br-elcos-cli in frame {&frame-name}.
    end.
  ...
  apply "entry" to br-elcos-cli in frame {&frame-name}.
END.
...
&scop flt-open-open-query         OPEN QUERY br-elcos-cli FOR EACH X_ext-classif

&scop flt-open-dyn_open-query     FOR EACH X_ext-classif

&scop flt-open-query-handle      QUERY br-elcos-cli:handle
...
title0 = "Клиенты в системе ЭЛКОС ТАЛОН".
...
{ gbl/fltopend.i
        &where-cond = " X_ext-classif.classif-subject = ~{&table_clients~} ~
                        and X_ext-classif.classif-name = ~{&extclass_clients_elcos~} ~
        ...
        &dyn_where-cond = " substitute('X_ext-classif.classif-subject = &1&2&1 ~
                        ...
                        AND X_ext-classif.db-num = - 1', {&double-quote}, ~{&table_clients~}, ~{&extclass_clients_elcos~})"
...
define buffer buf_clients for ub.clients.
message
"Выберите клиента, для которого Вы хотите добавить запись в системе ЭЛКОС-ТАЛОН"
view-as alert-box .
...
run gbl/d-integer.w (
       input ?
      ,input (
      'title=':u + substitute("Добавление кода клиента в системе ЭЛКОС-ТАЛОН для &1&2"
                              ,buf_clients.obj-type
                              ,buf_clients.obj-code) + '\':u
    + 'text1=':u + "Код клиента в систем ЭЛКОС-ТАЛОН" + '\':u
    + 'format=' + ">>>>>>>>9" + '\':u
    + 'fillin_row=2\':u
    + 'fillin_col=30\':u
    + 'fillin_width=10\':u
...
run ref/extclas1.p ( INPUT {&add-def}
                    ...
                    ,INPUT {&extclass_clients_elcos} /*p-classif-name*/
                    ,input (-1) /*p-db-num*/
...
if v-rid <> ? then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-elcos-cli to recid v-rid no-error .
  apply "entry" to br-elcos-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
if not error-status:error then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-elcos-cli to row 1 no-error .
  apply "entry" to br-elcos-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
DO WHILE available X_ext-classif :
   GET prev br-elcos-cli.
END.
GET next br-elcos-cli.
DO WHILE available X_ext-classif :
...
run fltfield-add in this-procedure('key#_one', 'Код клиента Элкос-талон', '',
...
DO on stop undo, leave:
    ...
    run OpenBr IN THIS-PROCEDURE ( INPUT YES, INPUT NO, INPUT '':U).
    if v-ri <> ? then do:
      reposition br-elcos-cli to recid v-ri no-error.
    end.
    APPLY "ENTRY" to br-elcos-cli in frame {&frame-name} .
    APPLY "VALUE-CHANGED" to br-elcos-cli.
END .
