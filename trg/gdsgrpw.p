...
if v-value = "no"  then do:   
  ...
  if ub.gds-grp.node-code <> oldb.node-code then do:
    /* !!! */
    ...
    :
      ...
      if not available buf2_scales-grp then do:
    ...
    on error undo main-block, return error return-value  :
      ...
      if not available buf2_fbr-prn-grp then do:
    ...
    if ub.gds-grp.calc-method = "" then do:
      assign
  ...
  else if ub.gds-grp.upper-code <> oldb.upper-code then do:
    find b-gds-grp  where
        b-gds-grp.node-code = ub.gds-grp.upper-code no-wait no-error.
    if not available b-gds-grp then do:
...
procedure grp-tree :
  ...
  :
    ...
    :
      if nc = ub.gds-grp.node-code then do:
        assign
