...
:
  ...
  if not new(ub.clients) then do:
    /* код и тип клиента менять нельзя */
    if ub.clients.obj-type <> old-clients.obj-type
    or ub.clients.obj-code <> old-clients.obj-code then do:
      message
    ...
    if ub.clients.obj-code = ? then do:
      message
        vss-workfile vss-revision vss-description skip
    ...
    if ub.clients.obj-code = 0 then do:
      message
        vss-workfile vss-revision vss-description skip
        "Не задан код клиента" skip
        "ub.clients.obj-code" ub.clients.obj-code skip
  ...
  then do:
    ...
    if not new(ub.clients) then do:
      if ub.clients.db-num <> old-clients.db-num then do:
  ...
  if ub.clients.grp-code = ? then do:
    message
      vss-workfile vss-revision vss-description skip
  ...
  find first ub.cli-grp no-lock
    ...
    no-error .
  if not available ub.cli-grp then do:
    message
      vss-workfile vss-revision vss-description skip
  ...
  if
  not g#news and
  not (ub.clients.obj-type = {&shop} or ub.clients.obj-type = {&stock}) then do:
     { ref/send-ref.i conf-par par-type }
    /*создаем batchporcess для отсылки на кассы*/
    if send-ref then do:
      if not new(ub.clients) then do:
        /*выясним что изменилось*/
        if lookup("obj-name", v-l-chr) > 0 then do:
    ...
    If ub.clients.obj-type = {&prs} then do:
      if old-clients.obj-name <> ub.clients.obj-name then do:
        assign
  ...
  if not g#news and g#db-num = 0 then do:
     if ub.clients.turnover-buyer =  true  and  old-clients.turnover-buyer = false then do:
        run ref/calcturn.p (ub.clients.obj-type, ub.clients.obj-code) no-error .
     end.
     if ub.clients.turnover-buyer =  false and  old-clients.turnover-buyer = true  then do:
